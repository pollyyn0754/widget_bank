  ...
  + `read_xlsx_transactions`, которая принимает на вход путь до xlsx-файла и возвращает двумерную таблицу
    ...
    def read_xlsx_transactions(filepath: str, sheet_name=0) -> pd.DataFrame:
        ...
        try:
            ...
            return df
    ```
+ В модуле `main` реализована функция `main`, которая отвечает за основную логику проекта и связывает 
  функциональности между собой. Ожидаемое поведение программы должно быть следующим:
  ```
...
Отчет о тестировании хранится в пакете `htmlcov`.